...
impl<'prl, 'mm> JxlEncoder<'prl, 'mm> {
    ...
    #[builder]
    pub fn new(memory_manager: Option<&'mm dyn MemoryManager>, 
    #[builder(default)]
    has_alpha: bool,
...
use jxl_encoder_builder::{State, IsUnset, SetQuality};
...
impl JxlEncoder<'_, '_> {
    ...
    fn set_options(&self) -> Result<(), EncodeError> {
        self.check_enc_status(unsafe { JxlEncoderUseContainer(self.enc, self.use_container.into()) })?;
        if let Some(lossless) = self.lossless {
            self.check_enc_status(unsafe { JxlEncoderSetFrameLossless(self.options_ptr, lossless.into()) })?;
        }
