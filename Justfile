...
jpegxl-sys v$(cargo metadata --format-version=1 | \
    jaq -r '.packages[] | select(.name | contains("jpegxl-sys")) | .version')\

jpegxl-src v$(cargo metadata --format-version=1 | \
    jaq -r '.packages[] | select(.name | contains("jpegxl-src")) | .version')"
